...
```
Doing this reminded me of how to link my JavaScript code file on a HTML page using the ```<script tag>``` and assigning the JavaScript filen to the ```src``` attribute as seen above. The JavaScript code that has the game implementation was stored in ```battleship.js``` file.
